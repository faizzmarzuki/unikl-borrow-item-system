...
if(isset($_POST['btnsave']))
{
  ...
  `Type`='$Type',`Other`='$Other',`Model1`='',`Serial_num1`='',`Model2`='',`Serial_num2`='',`Model3`='',`Serial_num3`='',`Model4`='',`Serial_num4`='',`Justify`='$Justify', `Date_Start`='$Date_Start',
  `Date_End`='$Date_End' ,`Review`='',`sName`='',`LName`='$LName',`LReview`='$LReview' WHERE `id`='$id'";
	...
	if($sql)
      ...
      if($LReview=='Approved'){
        $mail = new PHPMailer(true);
            ...
            $mail->Body    = 'Your loan already been approved by your Supervisor.<br>
          ...
          $mail = new PHPMailer(true);
            ...
            //Recipients
            $mail->setFrom('no-reply@unikl.org', 'no-reply@unikl');
            $mail->addAddress('hashim.mamat99@gmail.com', $Name);     // Add a recipient
    
            // Content
            $mail->isHTML(true);                                  // Set email format to HTML
            $mail->Subject = 'Announcement on '.$Name.' loan has been approved';
            $mail->Body    = $Name.' loan already been approved by his/her Supervisor.<br>
                              Please update his/her data.<br>
                              Thank You,<br>
                              IT Unit.';
            $mail->AltBody = 'This is the body in plain text for non-HTML mail clients';
        ...
        $mail = new PHPMailer(true);
          ...
          $mail = new PHPMailer(true);
            ...
            // Content
            $mail->isHTML(true);                                  // Set email format to HTML
            $mail->Subject = 'Announcement on '.$Name.' loan did not approved';
            $mail->Body    = $Name.' loan already did not approved by his/her Supervisor.<br>
                              Please update his/her data.<br>
                              Thank You,<br>
                              IT Unit.';
            $mail->AltBody = 'This is the body in plain text for non-HTML mail clients';
	  ...
	  <b>[Laptop]</b>
		...
		<b>[Others]</b>
          <div class="preferred-metod-item">
			<input type="text" name="txttype" size="100%" value="<?php echo $rec['Type']; ?>">
		  </div>
		 <br>
		<div class="item">
          ...
          <div class="name-item">
            <input type="text" name="model1" value="<?php echo $rec['Model1']; ?>"READONLY>
            <input type="text" name="num1" value="<?php echo $rec['Serial_num1']; ?>"READONLY>
			<input type="text" name="model2" value="<?php echo $rec['Model2']; ?>"READONLY>
            <input type="text" name="num2" value="<?php echo $rec['Serial_num2']; ?>"READONLY>
			<input type="text" name="model3" value="<?php echo $rec['Model3']; ?>"READONLY>
            <input type="text" name="num3" value="<?php echo $rec['Serial_num3']; ?>"READONLY>
			<input type="text" name="model4" value="<?php echo $rec['Model4']; ?>"READONLY>
            <input type="text" name="num4" value="<?php echo $rec['Serial_num4']; ?>"READONLY>
          </div>
        </div>
		 <div class="item">
          <p>Justification</p>
		...
		<div class="item">
          <p>Borrow Date</p>
          <div class="name-item">
            <input type="date" name="start" value="<?php echo $rec['Date_Start']; ?>">
			<center><font size="4">to</font></center>
            <input type="date" name="end" value="<?php echo $rec['Date_End']; ?>"> 
          </div>
        </div>
...
<div class="name-item">
<input type="text" name="LReview" value="<?php echo $rec['LReview']; ?>"> 
<input type="text" placeholder="Type your name....." name="LName" value="<?php echo $rec['LName']; ?>">
</div>
</div>
</fieldset>
...
<p>Please type in the textbox <b>[Approved] or [Not Approved]</b></p>
<div class="name-item">
<input type="text" name="Review" value="<?php echo $rec['Review']; ?>" READONLY> 
<input type="text" placeholder="Type your name....." name="sName" value="<?php echo $rec['sName']; ?>" READONLY>
</div>
</div>
</fieldset>

<td><center><button class="qa" type="submit" value="Save" name="btnsave" >Save</button></center></td>
		  <script>
			function myFunction() {
